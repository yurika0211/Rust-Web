...
pub async fn post_new_course_db(pool: &PgPool, new_course: CreateCourse) -> Result<Course, MyError> { 
    let row = sqlx::query_as!(
        Course,
        r#"insert into course (teacher_id, name, description, format, structure, duration, price, language, level) values ($1, $2, $3, $4, $5, $6, $7, $8, $9)
...
// 从数据库当中删除指定课程
pub async fn delete_course_db(pool: &PgPool, teacher_id: i32, course_id: i32)-> Result<String, sqlx::Error> {
